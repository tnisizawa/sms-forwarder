...
先にスプレッドシートの「SMS転送」→「**テスト送信**」を実行すると、実際の SMS を待たずに
転送先へテストメールが 1 通届き、`log` シートにも 1 行記録されます（filter で落ちる内容ならその旨が表示されます）。

そのあと別の電話から SMS を1通送り、`[SMS] <番号>` という件名のメールが届けば完了です。
同じ番号からのメールは Gmail で1つのスレッド（会話）にまとまり、番号が違うと別スレッドになります。
